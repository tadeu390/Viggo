<?php
	...
	*/
	class Nota_model extends Geral_model 
	{
		...
		*/
		public function get_nota($descricao_nota_id, $matricula_id, $turma_id, $disciplina_id, $etapa_id)
		{
			$query = $this->db->query("
		...
		*	$etapa_id -> Id do bimestre para o qual se deseja inserir a nota.
		*/
		public function set_notas($nota, $descricao_nota_id, $matricula_id, $turma_id, $disciplina_id, $etapa_id)
		{
			//verificar se já existe essa nota no banco para os parâmetros informados acima
			$query = $this->get_nota($descricao_nota_id, $matricula_id, $turma_id, $disciplina_id, $etapa_id);
		...
		*
		*	$matricula_id -> Matricula do aluno.
		*	$etapa_id -> Id do bimestre para o qual se deseja saber o total de nota.
		*	$turma_id -> Id da turma na qual se encontra o aluno.
		*	$disciplina_id -> Id da disciplina.
		* 	$nota -> Nota enviada pelo usuário.
		*	$descricao_nota_id -> Tipo de nota adicionada.
		*/
		public function validar_nota($matricula_id, $etapa_id, $turma_id, $disciplina_id, $nota, $descricao_nota_id)
		{
			if($nota == null)
				$nota = 0;
			//caso esteja alterando a nota, busca o valor já cadastrado no banco.
			$nota_banco = $this->get_nota($descricao_nota_id, $matricula_id, $turma_id, $disciplina_id, $etapa_id)['Nota'];
		...
		*	$etapa_id -> Id do bimestre para o qual se deseja saber o total de nota.
		*/
		public function total_nota($matricula_id, $turma_id, $disciplina_id, $etapa_id)
		{
			...
			$query = $this->db->query("
				...
				INNER JOIN Disc_grade dg ON dt.Disc_grade_id = dg.Id 
				INNER JOIN Matricula m ON m.Disc_turma_id = dt.Id 
				INNER JOIN Notas n ON n.Matricula_id = m.Id AND n.Matricula_id = ".$this->db->escape($matricula_id)." AND n.etapa_id = ".$this->db->escape($etapa_id)." 
		...
		*/
		public function status_nota_total_etapa($matricula_id, $turma_id, $disciplina_id, $etapa_id, $periodo_letivo_id)
		{
			...
			$val_bimestre = (!empty($CI->Etapa_model->get_etapa(FALSE, $etapa_id, FALSE)['Valor']) ? $CI->Etapa_model->get_etapa(FALSE, $etapa_id, FALSE)['Valor'] : 0);
